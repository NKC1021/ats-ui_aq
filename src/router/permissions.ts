...
export function setupPermissions(router: Router) {
  ...
  router.beforeEach(async (to: { path: string }, from: any, next: any) => {
    ...
    const { getTimeList, getPeopleList, getAllDetailsByARS } = recordUserParma()
    const { getKeyCodeForm, getLogoBySet } = recordProInfo()
    ...
    if (hasToken) {
      if (routes.length) {
        ...
        if (to.path === '/login') {
          ...
          const removeTabs = ['/myProject', '/shareProject']
          if (removeTabs.includes(to.path)) {
            tabsStore.visitedRoutes = tabsStore.visitedRoutes.filter((el) => {
              return removeTabs.includes(el.path)
            })
          }
          next()
        }
        ...
        try {
          ...
          // await getTimeList()
          // await getPeopleList()
          // await getAllDetailsByARS()
          // console.log('登录成功后第一个路由:::', to)
