...
export const diff = (obj1, obj2) => {
  ...
  // V1.3 label相同即认为是同一个路由， 不再进行详细的查询条件等判断
  /* if (obj1['label'] === obj2['label']) {
    return true;
  } */
  const path1 = getPath(obj1['value'])
  const path2 = getPath(obj2['value'])
...
export const decryption = (data, key) => {
  ...
  const decrypt = CryptoJS.AES.decrypt(data, iv, {
    iv: iv,
 ...
 * @returns
 */
export const strEncryption = (data, key) => {
  const iv = CryptoJS.enc.Latin1.parse(key)
...
/*递归寻找某一属性相等时当前的item */
export const findItem = (menu, key, val) => {
  // if (!menu) {
  //   router.push({
  //     path: '/login'
  //   });
  ...
  for (const item of menu) {
    ...
    if (item.children.length > 0) {
      const res = findItem(item.children, key, val)
      if (res) {
        return res
      }
    }
...
//     if (blob && blob.size === 0) {
//       this.$notify.error(translate('i18_content_is_empty_can_not_download'))
//       return
//     }
 ...
 * @returns
 */
export function isArray1ContainsArray2(array1, array2) {
  return array2 && array2.some((element) => array1.includes(element))
}
