 ...
 * @LastEditTime: 2023-04-18 20:27:57
  ...
  <div class="echarts-container ai-filter">
    ...
    <div class="query-form">
      <query-form
        ...
        @HandleIsShowTable="HandleIsShowTable"
        @getData="getData"
...
<script setup>
  ...
  const tableData = ref([])
  option.value.title.text = translate('AI过滤-设备')
  const getData = (data) => {
    isShowTable.value = data.isShowTable
    if (!isShowTable.value) {
      echartData.value = [...data.data]
      sortData(echartData.value, 'equipment', setForm.value)
      initDrawSeries(option.value, echartData.value, setForm.value, 'equipment')
